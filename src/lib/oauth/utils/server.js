...
// ───────────────────────────────────────────────────────────────────────────
// Z.AI fixed-port proxy on 127.0.0.1:19765
// Same shape as the xAI proxy (fixed port keeps redirect_uri stable for
// chat.z.ai's authorize endpoint). No PKCE — state is the only session key.
// Mode A (server-side): registered session → auto-exchange + save DB.
// Mode B (fallback): 302 to the app's /callback relay page.
// ───────────────────────────────────────────────────────────────────────────

let zaiProxyServer = null;
let zaiProxyTimeout = null;
const ZAI_PROXY_TIMEOUT_MS = 300000; // 5 minutes
const ZAI_PROXY_PORT = 19765;
const zaiPendingExchanges = new Map();

export function registerZaiSession({ state, redirectUri }) {
  if (!state || !redirectUri) return false;
  zaiPendingExchanges.set(state, {
    redirectUri,
    status: "pending",
    createdAt: Date.now(),
  });
...
export function getZaiSessionStatus(state) {
  return zaiPendingExchanges.get(state) || null;
}

export function clearZaiSession(state) {
  zaiPendingExchanges.delete(state);
}

export function startZaiProxy(appPort) {
  return new Promise((resolve) => {
    if (zaiProxyServer) {
      resolve({ success: true });
      return;
    ...
    const server = http.createServer(async (req, res) => {
      ...
      const state = url.searchParams.get("state");
      const errorParam = url.searchParams.get("error");
      const session = state ? zaiPendingExchanges.get(state) : null;
      ...
      if (session) {
        try {
          ...
          const tokenData = await exchangeTokens("zai", code, session.redirectUri, null, state);
          const connection = await createProviderConnection({
            provider: "zai",
            authType: "oauth",
            ...tokenData,
      ...
      // Mode B: legacy fallback redirect to the app's relay page
      const redirectUrl = `http://localhost:${appPort}/callback${url.search}`;
      res.writeHead(302, { Location: redirectUrl });
      res.end();
      stopZaiProxy();
    ...
    server.listen(ZAI_PROXY_PORT, "127.0.0.1", () => {
      zaiProxyServer = server;
      zaiProxyTimeout = setTimeout(() => stopZaiProxy(), ZAI_PROXY_TIMEOUT_MS);
      resolve({ success: true });
    });
...
export function stopZaiProxy() {
  if (zaiProxyTimeout) {
    clearTimeout(zaiProxyTimeout);
    zaiProxyTimeout = null;
  }
  if (zaiProxyServer) {
    zaiProxyServer.close();
    zaiProxyServer = null;
  }
}
